...
def HVCs(intnsr):
    ...
    pool99 = tf.nn.avg_pool2d(intnsr, shape[1], shape[2])
    caps_input = pool99
    cap_dims   = 8
...
def caps_from_conv_zxy(in_tensor, cap_count, cap_dims):
    x = tf.transpose(in_tensor, [0, 3, 1, 2])
    x = tf.reshape(x, [-1, 1, cap_count, cap_dims])
    return x

def batch_norm(is_training,in_tensor, variable, decay=0.9997, epsilon=0.001):
    if is_training:
    ...
    return tf.nn.batch_normalization(in_tensor, mean,
            variance, variable.bn_beta, None, epsilon)


def hvc_from_zxy(is_training, in_tensor, variable):
        x = tf.reduce_sum(tf.multiply(in_tensor, variable.variable), 2)
        x = batch_norm(is_training, x, variable)
        x=squash(x)
        return tf.nn.relu(x)

class VariableWBatchNorm(object):
    def __init__(self, initial_value, trainable=True, bn_shape_idx=-1):
...
def get_batch_norm_variable(shape, initializer=tf.random_normal_initializer(),
        trainable=True, bn_shape_idx=-1):
    return VariableWBatchNorm(initializer(shape=shape),trainable=trainable, bn_shape_idx=bn_shape_idx)

def get_conv_batch_norm_variable(input_size, filter_size_h, filter_size_w, filters):
    return get_batch_norm_variable(
        [filter_size_h, filter_size_w, input_size, filters])

def get_conv_3x3_batch_norm_variable(input_size, filters):
    return get_conv_batch_norm_variable(input_size,3,3, filters)

def get_hvc_from_zxy_batch_norm_variable(input_size, output_size, capsule_dimensions):

    return get_batch_norm_variable(
        [output_size, input_size, capsule_dimensions], bn_shape_idx=-1
    )
